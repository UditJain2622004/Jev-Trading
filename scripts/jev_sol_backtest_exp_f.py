"""Experiment F: no Jev. Buy follow-through from the snapshot.

Copied from scripts/jev_sol_backtest_exp_e.py. Same stretch skip. Then buy
only if 15-minute buyer share is at least 0.55 and at least 8 of the last
15 minutes were up. Tests whether the extra now-card fields have an edge
without the model.

    python -u scripts/jev_sol_backtest_exp_f.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_f")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment F: snapshot follow-through, no Jev")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_f/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_f")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_f_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_f")
...
def main() -> None:
    ...
    log.info("Experiment F: stretch skip then buy follow-through (no Jev)")
