"""Experiment N: skill/playbook text in state, 3+3 examples.

Copied from scripts/jev_sol_backtest_exp_l.py. Same two Choice questions
and 3 recent wins / 3 recent losses. Adds a written playbook in `state.skill`
that says when to pick each answer, like a skill file for an LLM.

    python -u scripts/jev_sol_backtest_exp_n.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_n")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment N: skill text in state")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_n/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_n")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_n_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_n")
...
SKILL = {
    "role": (
        "You are judging whether a 60-minute SOL long is a good bet. "
        "A win pays about $3 after fees. A stop loses about $5.50. "
        "A maybe is not enough. Only say the long looks good when the tape clearly agrees."
    ),
    "looks_like": {
        "more_like_wins": (
            "Use this when `now` shares several traits with `recent_wins`: "
            "buyers still in control, the 15-minute path is up without being spent, "
            "and there is room to the hour high. One similar field is not enough."
        ),
        "more_like_losses": (
            "Use this when `now` shares traits with `recent_losses`: "
            "already near the hour high, a spike that just happened, fading buy pressure, "
            "or a close stuck at the highs. When in doubt, pick this."
        ),
        "unclear": (
            "Use this when `now` does not clearly match either list, "
            "or the two lists disagree. Do not force a resemblance."
        ),
    },
    "hit_tp_first": {
        "1": (
            "Pick 1 only if a +0.5% rise is more plausible than a -0.35% drop inside 60 minutes, "
            "AND this is not a late chase. Need persistence, room, and buyers still active."
        ),
        "0": (
            "Pick 0 if the long is late, stretched, fading, Bitcoin is dumping, "
            "volume is dead, or you would not bet $5.50 to make $3. "
            "If unsure, pick 0."
        ),
    },
    "do_not": [
        "Do not treat the example lists as a 50/50 base rate. Most raw longs lose.",
        "Do not answer 1 just because the last few minutes were green.",
        "Do not ignore `skill` and guess from the numbers alone.",
    ],
}
...
def build_state(
    ...
    return {
        ...
        "plan": {
            ...
            "stop_pct": 0.35,
            "timeout_minutes": 60,
            "note": "Follow `skill` when answering. Examples are recent finished trades, 3 each.",
        },
        "now": now,
        "recent_wins": wins,
...
def build_questions(*, has_examples: bool) -> dict[str, object]:
    ...
    return {
        "looks_like": Choice(
            instructions={
                "question": "Does `now` look more like `recent_wins` or `recent_losses`?",
                "follow": "state.skill.looks_like",
                "also_read": ["state.skill.role", "state.skill.do_not"],
            },
            criteria={
                "more_like_wins": "Matches skill.looks_like.more_like_wins",
                "more_like_losses": "Matches skill.looks_like.more_like_losses",
                "unclear": "Matches skill.looks_like.unclear",
            },
        ),
        "hit_tp_first": Choice(
            instructions={
                "question": "If we buy SOL now, does +0.5% come before -0.35% inside 60 minutes?",
                "follow": "state.skill.hit_tp_first",
                "also_read": ["state.skill.role", "state.skill.do_not"],
            },
            criteria={
                "0": "Matches skill.hit_tp_first.0",
                "1": "Matches skill.hit_tp_first.1",
            },
        ),
    }
...
def main() -> None:
    ...
    log.info("Experiment N: skill playbook in state, 3+3 examples")
