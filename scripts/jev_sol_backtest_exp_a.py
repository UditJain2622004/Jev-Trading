"""Experiment A: same Jev questions, rolling recent examples.

Copied from scripts/jev_sol_backtest.py. The only lever that changed is
which examples Jev sees. Before each test trade we send the most recent
10 profitable and 10 unprofitable trades that have already finished
(exit time before this entry). Timeouts count by net dollars, not by
hit/miss. Examples from the same 30-minute cluster are skipped so we
do not send ten copies of one move.

    python -u scripts/jev_sol_backtest_exp_a.py --dry-run
    python -u scripts/jev_sol_backtest_exp_a.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_a")
EXAMPLE_GAP_MS = 30 * 60 * 1000


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment A: rolling recent Jev examples")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_a/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_a")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_a_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_a")
...
def main() -> None:
    ...
    log.info("Experiment A: rolling 10 profitable / 10 unprofitable examples")
    ...
    if args.dry_run:
        ...
        log.info(
            "Dry run first-test examples: profitable=%s unprofitable=%s",
            len(pos),
            len(neg),
        )
    ...
    try:
        with results_path.open(mode, encoding="utf-8") as handle:
            for n, trade in enumerate(pending, start=1):
                ...
                win_examples = [{**t.example, "pnl_usd": round(t.pnl_usd, 2)} for t in pos]
                loss_examples = [{**t.example, "pnl_usd": round(t.pnl_usd, 2)} for t in neg]
                ...
                if n % 10 == 0 or n == len(pending):
                    ...
                    log.info(
                        "Progress %s/%s  elapsed %.0fs  ~%.0fs left  rolling pos=%s neg=%s newest_pos=%s",
                        n,
                        len(pending),
                        elapsed,
                        left,
                        len(pos),
                        len(neg),
                        pos[-1].entry_ms if pos else None,
                    )
                if args.sleep_ms:
