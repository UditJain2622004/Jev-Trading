"""Experiment E: skip only late/stretched entries, then Jev as C.

Copied from scripts/jev_sol_backtest_exp_d.py. D's "must look alive" AND-gate
selected 20 slots that were 5 wins / 15 losses. E only blocks when price is
already into the hour high (room under 0.15%). Quiet or down-tape slots still
go to Jev.

    python -u scripts/jev_sol_backtest_exp_e.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_e")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment E: skip stretched entries then Jev")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_e/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_e")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_e_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_e")
...
def main() -> None:
    ...
    log.info("Experiment E: skip stretched-into-high only, then Jev")
