"""Experiment B: rolling examples plus Jev's own right/wrong calls.

Copied from scripts/jev_sol_backtest_exp_a.py. Rolling 10 profitable /
10 unprofitable market examples stay. The new lever is four extra
lists, 3 each, built only from earlier Jev calls that have already
finished: correct buy, wrong buy, correct skip, wrong skip.

    python -u scripts/jev_sol_backtest_exp_b.py --dry-run
    python -u scripts/jev_sol_backtest_exp_b.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_b")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment B: rolling examples plus Jev feedback")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_b/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_b")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_b_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_b")
    ...
    return {
        ...
        "hit_tp_first": Choice(
            instructions=(
                ...
                "Use `recent_wins`, `recent_losses`, and the four feedback lists. "
                "Pick exactly 0 or 1."
            ),
            criteria={
...
def main() -> None:
    ...
    log.info("Experiment B: rolling market examples plus 4 Jev feedback buckets")
