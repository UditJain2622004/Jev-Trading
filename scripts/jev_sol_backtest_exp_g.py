"""Experiment G: no Jev. Buy pullbacks from the snapshot.

Copied from scripts/jev_sol_backtest_exp_f.py. Same stretch skip. Then buy
if room to hour high is at least 0.40%, at most 6 of the last 15 minutes
were up, and 15-minute buyer share is still at least 0.48. Invert of F.

    python -u scripts/jev_sol_backtest_exp_g.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_g")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment G: snapshot pullbacks, no Jev")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_g/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_g")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_g_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_g")
...
def snapshot_pullback(trade: ReplayTrade) -> JevDecision:
    snap = trade.snapshot
    hour = snap.get("sol_last_hour") if isinstance(snap.get("sol_last_hour"), dict) else {}
    ...
    if room is None or share is None or up is None:
        return JevDecision(False, "missing pullback fields", "0", None)
    if float(room) >= 0.40 and int(up) <= 6 and float(share) >= 0.48:
        return JevDecision(True, "pullback with buyers", "1", None)
    return JevDecision(False, "not a pullback", "0", None)
...
def main() -> None:
    ...
    log.info("Experiment G: stretch skip then buy pullbacks (no Jev)")
    ...
    with results_path.open(mode, encoding="utf-8") as handle:
        for n, trade in enumerate(pending, start=1):
            ...
            if blocked:
                ...
                decision = snapshot_pullback(trade)
