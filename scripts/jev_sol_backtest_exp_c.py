"""Experiment C: richer now-card, same examples as B.

Copied from scripts/jev_sol_backtest_exp_b.py. Rolling 10/10 market
examples and the four feedback buckets stay. The new lever is extra
fields on the entry snapshot: path, volatility, buying pressure, last
candle wicks, volume acceleration, VWAP distance, UTC hour/weekday.

    python -u scripts/jev_sol_backtest_exp_c.py --dry-run
    python -u scripts/jev_sol_backtest_exp_c.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_c")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment C: richer now-card")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_c/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_c")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_c_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_c")
...
def main() -> None:
    ...
    log.info("Experiment C: richer now-card on top of rolling examples + feedback")
