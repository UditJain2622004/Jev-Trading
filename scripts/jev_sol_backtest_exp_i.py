"""Experiment I: same state as C, but use Jev probabilities.

Copied from scripts/jev_sol_backtest_exp_c.py. Questions and context stay.
The live buy rule is pre-registered: P(hit_tp_first=1) >= 0.60 and
P(more_like_wins) > P(more_like_losses). Full distributions are saved so
other thresholds can be scored after the run without more Jev calls.

    python -u scripts/jev_sol_backtest_exp_i.py
"""
...
from jev_trading.sol_jev.answers import choice_p, dump_answers
...
log = logging.getLogger("jev_sol_backtest_exp_i")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment I: probability logging and calibration")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_i/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_i")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_i_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_i")
...
def summarize_calls(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    for row in rows:
        ...
        if predicted_win == actual_win:
            ...
            wrong += 1
    resolved = by_take.get("win", 0) + by_take.get("loss", 0)
    return {
        ...
        "jev_right": right,
        "jev_wrong": wrong,
        "resolved_win_rate_pct": round(100.0 * by_take.get("win", 0) / resolved, 2) if resolved else None,
    }


def decide_from_probs(dumped: dict[str, object]) -> JevDecision:
    p_hit = choice_p(dumped, "hit_tp_first", "1")
    p_wins = choice_p(dumped, "looks_like", "more_like_wins")
    p_losses = choice_p(dumped, "looks_like", "more_like_losses")
    hit = None
    looks = None
    hit_item = dumped.get("hit_tp_first")
    looks_item = dumped.get("looks_like")
    if isinstance(hit_item, dict):
        hit = str(hit_item.get("choice")) if hit_item.get("choice") is not None else None
    if isinstance(looks_item, dict):
        looks = str(looks_item.get("choice")) if looks_item.get("choice") is not None else None
    if p_hit is None:
        return JevDecision(False, "missing hit_tp_first probability", hit, looks)
    if p_wins is None or p_losses is None:
        return JevDecision(False, "missing looks_like probability", hit, looks)
    if p_wins <= p_losses:
        return JevDecision(False, "P(wins) not above P(losses)", hit, looks)
    if p_hit < HIT_P_MIN:
        return JevDecision(False, f"P(hit=1) {p_hit:.2f} < {HIT_P_MIN:.2f}", hit, looks)
    return JevDecision(True, f"P(hit=1)={p_hit:.2f} P(wins)={p_wins:.2f}", hit, looks)


def policy_summary(rows: list[dict[str, object]], takes: list[bool]) -> dict[str, object]:
    selected = [row for row, take in zip(rows, takes) if take]
    skipped = [row for row, take in zip(rows, takes) if not take]
    by_take: Counter[str] = Counter(str(r["outcome"]) for r in selected)
    resolved = by_take.get("win", 0) + by_take.get("loss", 0)
    return {
        "takes": len(selected),
        "skips": len(skipped),
        "net_usd": round(sum(float(r["pnl_usd"]) for r in selected), 4),
        "wins": by_take.get("win", 0),
        "losses": by_take.get("loss", 0),
        "timeouts": by_take.get("timeout", 0),
        "resolved_win_rate_pct": round(100.0 * by_take.get("win", 0) / resolved, 2) if resolved else None,
    }
...
def calibration_table(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for thr in (0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80):
        takes = []
        for row in rows:
            dumped = row.get("jev_answers") if isinstance(row.get("jev_answers"), dict) else {}
            p_hit = choice_p(dumped, "hit_tp_first", "1")
            p_wins = choice_p(dumped, "looks_like", "more_like_wins")
            p_losses = choice_p(dumped, "looks_like", "more_like_losses")
            ok = p_hit is not None and p_hit >= thr
            if p_wins is not None and p_losses is not None:
                ok = ok and p_wins > p_losses
            takes.append(ok)
        item = {"threshold": thr, "gate": "P(hit=1)>=thr and P(wins)>P(losses)"}
        item.update(policy_summary(rows, takes))
        table.append(item)
    for thr in (0.40, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75):
        takes = []
        for row in rows:
            dumped = row.get("jev_answers") if isinstance(row.get("jev_answers"), dict) else {}
            p_hit = choice_p(dumped, "hit_tp_first", "1")
            takes.append(p_hit is not None and p_hit >= thr)
        item = {"threshold": thr, "gate": "P(hit=1)>=thr only"}
        item.update(policy_summary(rows, takes))
        table.append(item)
    return table
...
def main() -> None:
    ...
    log.info("Experiment I: C state/questions, buy on P(hit=1)>=%.2f and P(wins)>P(losses)", HIT_P_MIN)
        ...
        with results_path.open(mode, encoding="utf-8") as handle:
            for n, trade in enumerate(pending, start=1):
                ...
                try:
                    ...
                    decision = decide_from_probs(dumped)
    ...
    summary["live_threshold"] = HIT_P_MIN
    summary["calibration"] = calibration
    for item in calibration:
        log.info(
            "cal %s thr=%.2f  takes=%s  W/L/T=%s/%s/%s  net=$%.2f  resolved=%.1f%%",
            item["gate"],
            item["threshold"],
            item["takes"],
            item["wins"],
            ...
            item["timeouts"],
            item["net_usd"],
            item["resolved_win_rate_pct"] or 0.0,
        )
    ...
    log.info("Wrote %s", out_dir / "calibration.json")
