"""Experiment L: example ablation. Same C questions, two states.

Copied from scripts/jev_sol_backtest_exp_c.py. Same test window. No
feedback buckets. Mode `none` sends zero examples. Mode `minimal` sends
3 recent wins and 3 recent losses. Default runs both.

    python -u scripts/jev_sol_backtest_exp_l.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_l")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment L: no-example vs 3+3 examples")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_l/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_l")
    parser.add_argument("--mode", choices=["all", "none", "minimal"], default="all")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_l_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_l")
...
def build_state(
    ...
    state: dict[str, object] = {
        "plan": {
            ...
            "stop_pct": 0.35,
            "timeout_minutes": 60,
            "note": "Judge `now` on its own market shape. Examples, if present, are recent finished trades.",
        },
        "now": now,
    }
    if wins:
        state["recent_wins"] = wins
    if losses:
        state["recent_losses"] = losses
    return state
...
def main() -> None:
    ...
    if args.mode == "all":
        ...
        for mode in ("none", "minimal"):
            cmd = [sys.executable, "-u", str(script), "--mode", mode]
            if args.max_test:
            ...
            if args.resume:
                cmd.append("--resume")
            print(f"Experiment L: starting mode={mode}", flush=True)
            subprocess.check_call(cmd)
        return

    sent_wins, sent_losses = (0, 0) if args.mode == "none" else (3, 3)
    args.out_dir = f"data/jev_backtest_exp_l/{args.mode}"
    args.log_file = f"data/jev_backtest_exp_l/{args.mode}/jev_sol_backtest.log"
    ...
    log.info("Experiment L mode=%s: send %s wins and %s losses, no feedback", args.mode, sent_wins, sent_losses)
