"""Experiment H: no Jev. Same follow-through rule as F, new trade shape.

Copied from scripts/jev_sol_backtest_exp_f.py. Stretch skip then buy if
15-minute buyer share >= 0.55 and at least 8 up-minutes. Replay is now
take-profit +0.8% / stop -0.15% so a ~37% resolved win rate breaks even
after 0.2% fees. F sat at ~44% on a $3 / -$5.50 payoff.

    python -u scripts/jev_sol_backtest_exp_h.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_h")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment H: follow-through on +0.8% / -0.15%")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_h/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_h")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_h_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_h")
...
def prefilter_block(trade: ReplayTrade) -> str | None:
    """Skip only if the long is already into the hour high."""
    snap = trade.snapshot
    ...
    if room is not None and float(room) < 0.15:
        return "prefilter: stretched into hour high"
    if close_in is not None and float(close_in) >= 0.9 and room is not None and float(room) < 0.25:
        return "prefilter: closed at highs"
    return None


def snapshot_follow_through(trade: ReplayTrade) -> JevDecision:
    snap = trade.snapshot
    ...
    share = pressure.get("15m")
    up = path15.get("up_minutes")
    if share is None or up is None:
        return JevDecision(False, "missing follow-through fields", "0", None)
    if float(share) >= 0.55 and int(up) >= 8:
        return JevDecision(True, "follow-through buyers", "1", None)
    return JevDecision(False, "no follow-through", "0", None)
...
def build_questions() -> dict[str, object]:
    ...
    return {
        ...
        "hit_tp_first": Choice(
            instructions=(
                "If we buy SOL now at `now.entry` with take-profit +0.8% and stop -0.15%, "
...
def history_from_jsonl(path: Path, test: list[ReplayTrade]) -> list[ScoredCall]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if trade is None:
            continue
        if str(row.get("reason", "")).startswith("prefilter"):
            continue
...
def main() -> None:
    ...
    log.info("Experiment H: follow-through on +0.8%% / -0.15%% (no Jev)")
    ...
    log.info("Replaying SOL +%.2f%% / -%.2f%% timeout=%sm on %s minutes", TP_PCT, SL_PCT, TIMEOUT_BARS, f"{len(sol):,}")
    ...
    if args.dry_run:
        ...
        jev_n = sum(1 for t in test if prefilter_block(t) is None)
        log.info("Dry run: not calling Jev. Test trades waiting: %s  prefilter would ask Jev on %s", len(test), jev_n)
        return
