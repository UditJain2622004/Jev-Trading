"""Experiment P: atomic Noul questions, composite score, 3+3 examples.

Copied from scripts/jev_sol_backtest_exp_j.py. Same eight Noul questions.
No feedback lists. Send 3 recent wins and 3 recent losses. Code scores:

    continuation - exhausted - late
    + 0.5*buyers + 0.5*rejected + 0.25*btc + 0.15*orderly + 0.15*vol

Live take if score >= 0.35. Also sweep thresholds after the fact.

    python -u scripts/jev_sol_backtest_exp_p.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_p")
EXAMPLE_GAP_MS = 30 * 60 * 1000
SCORE_MIN = 0.35


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment P: atomic Nouls, composite score, 3+3")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_p/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_p")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_p_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_p")
...
def summarize_calls(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "jev_wrong": wrong,
        "score_min": SCORE_MIN,
        "score_sweep": score_sweep(rows),
    }


def _policy_from_scores(rows: list[dict[str, object]], thr: float) -> dict[str, object]:
    takes = []
    for row in rows:
        score = row.get("composite_score")
        if score is None:
            dumped = row.get("jev_answers") if isinstance(row.get("jev_answers"), dict) else {}
            score, _ = composite_score(dumped)
        takes.append(float(score) >= thr)
...
def score_sweep(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    return [_policy_from_scores(rows, thr) for thr in (0.10, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60)]
...
def build_state(
    ...
    return {
        "plan": {
            ...
            "timeout_minutes": 60,
            "note": (
                "Answer each atomic question on its own. Code will combine the probabilities. "
                "`recent_wins` and `recent_losses` are 3 recent finished trades each."
            ),
        },
        "now": now,
        "recent_wins": wins,
...
def _noul(dumped: dict[str, object], key: str) -> float:
    value = noul_p(dumped, key)
    return 0.0 if value is None else value


def composite_score(dumped: dict[str, object]) -> tuple[float, dict[str, float]]:
    parts = {
        "continuation": _noul(dumped, "continuation"),
        "exhausted": _noul(dumped, "exhausted"),
        "late_entry": _noul(dumped, "late_entry"),
        "buyers_strengthening": _noul(dumped, "buyers_strengthening"),
        "downside_rejected": _noul(dumped, "downside_rejected"),
        "btc_supportive": _noul(dumped, "btc_supportive"),
        "regime_orderly": _noul(dumped, "regime_orderly"),
        "volatility_suitable": _noul(dumped, "volatility_suitable"),
    }
    score = (
        parts["continuation"]
        - parts["exhausted"]
        - parts["late_entry"]
        + 0.5 * parts["buyers_strengthening"]
        + 0.5 * parts["downside_rejected"]
        + 0.25 * parts["btc_supportive"]
        + 0.15 * parts["regime_orderly"]
        + 0.15 * parts["volatility_suitable"]
    )
    return score, parts


def decide_from_nouls(dumped: dict[str, object]) -> JevDecision:
    needed = ("continuation", "exhausted", "late_entry", "buyers_strengthening", "downside_rejected")
    if any(noul_p(dumped, key) is None for key in needed):
        return JevDecision(False, "missing noul answers", None, None)
    score, parts = composite_score(dumped)
    take = score >= SCORE_MIN
    reason = (
        f"score={score:.2f} {'>=' if take else '<'} {SCORE_MIN:.2f} "
        f"cont={parts['continuation']:.2f} exh={parts['exhausted']:.2f} late={parts['late_entry']:.2f}"
    )
    return JevDecision(take, reason, f"{score:.2f}", None)
...
def main() -> None:
    ...
    log.info("Experiment P: atomic Nouls, composite score >= %.2f, 3+3 examples", SCORE_MIN)
    ...
    if args.dry_run:
        ...
        log.info(
            "Dry run first-test examples: profitable=%s unprofitable=%s  no feedback",
            len(pos),
            len(neg),
        )
    ...
    try:
        with results_path.open(mode, encoding="utf-8") as handle:
            for n, trade in enumerate(pending, start=1):
                ...
                done_n += 1
                log.info(
                    "[%s/%s] %s  take=%s  score=%.2f  actual=%s  pnl=$%.2f  (%s)",
                    n,
                    len(pending),
                    trade.entry_ms,
                    decision.take,
    ...
    for item in summary.get("score_sweep") or []:
        log.info(
            "sweep thr=%.2f  takes=%s  net=$%.2f  W/L/T=%s/%s/%s  wr=%s",
            item["threshold"],
            item["takes"],
            item["net_usd"],
            item["wins"],
