"""Experiment K: semantic bucketed state, same C questions.

Copied from scripts/jev_sol_backtest_exp_c.py. Code converts the now-card
and examples into labels (rising/flat/falling, near_high, buyer_dominant, ...)
before sending them to Jev. Questions stay the two Choice questions from C.

    python -u scripts/jev_sol_backtest_exp_k.py
"""
...
from jev_trading.sol_jev.semantic import semantic_example, semantic_snapshot
...
log = logging.getLogger("jev_sol_backtest_exp_k")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment K: semantic bucketed state")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_k/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_k")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_k_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_k")
...
def market_payload(trade: ReplayTrade) -> dict[str, object]:
    return semantic_example(trade.snapshot, trade.outcome, trade.hold_minutes, trade.pnl_usd)


def feedback_payload(item: ScoredCall) -> dict[str, object]:
    profitable = item.trade.pnl_usd > 0.0
    payload = semantic_example(
        item.trade.snapshot,
        item.trade.outcome,
        item.trade.hold_minutes,
        item.trade.pnl_usd,
    )
    payload["we_bought"] = item.take
    payload["we_were_right"] = (item.take and profitable) or ((not item.take) and (not profitable))
    return payload
    ...
    return {
        "looks_like": Choice(
            instructions=(
                ...
                "Fields are semantic labels, not raw prices. "
        ...
        "hit_tp_first": Choice(
            instructions=(
                "If we buy SOL now with take-profit +0.5% and stop -0.35%, "
                "will price hit the take-profit before the stop within 60 minutes? "
                "Answer 0 if this long is already late (position_in_hour is near_high after a sharp run). "
                "Use the semantic labels in `now` and the example lists. "
                "Pick exactly 0 or 1."
            ),
            criteria={
...
def main() -> None:
    ...
    log.info("Experiment K: semantic labels instead of raw decimals")
    ...
    try:
        with results_path.open(mode, encoding="utf-8") as handle:
            for n, trade in enumerate(pending, start=1):
                ...
                state = build_state(
                    semantic_snapshot(trade.snapshot),
