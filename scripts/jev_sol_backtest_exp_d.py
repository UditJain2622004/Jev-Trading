"""Experiment D: prefilter dead slots, then same Jev as C.

Copied from scripts/jev_sol_backtest_exp_c.py. Context is unchanged.
Before calling Jev we skip quiet/stretched/down-tape entries:
volume below typical, less than 0.5% room to the hour high, or fewer
than 8 up-minutes in the last 15. Prefilter skips are not Jev feedback.

    python -u scripts/jev_sol_backtest_exp_d.py
"""
...
log = logging.getLogger("jev_sol_backtest_exp_d")
EXAMPLE_GAP_MS = 30 * 60 * 1000
FEEDBACK_PER_BUCKET = 3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Experiment D: prefilter then Jev")
    ...
    parser.add_argument("--log-file", default="data/jev_backtest_exp_d/jev_sol_backtest.log")
    parser.add_argument("--out-dir", default="data/jev_backtest_exp_d")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"jev_sol_backtest_exp_d_{stamp}.log")
    formatter = logging.Formatter("%(asctime)s  %(levelname)-7s  %(message)s", "%Y-%m-%d %H:%M:%S")
    stream = logging.StreamHandler(sys.stdout)
    stream.setFormatter(formatter)
    root = logging.getLogger("jev_sol_backtest_exp_d")
...
def prefilter_block(trade: ReplayTrade) -> str | None:
    """Skip dead slots before Jev. None means the slot is alive."""
    snap = trade.snapshot
    ...
    path15 = {}
    path = snap.get("path")
    if isinstance(path, dict) and isinstance(path.get("15m"), dict):
        path15 = path["15m"]
    vs = volume.get("vs_typical")
    room = hour.get("room_to_hour_high_pct")
    up = path15.get("up_minutes")
    blocked: list[str] = []
    if vs is None or float(vs) < 1.0:
        blocked.append("quiet volume")
    if room is None or float(room) < 0.5:
        blocked.append("no room to hour high")
    if up is None or int(up) < 8:
        blocked.append("few up minutes")
    if not blocked:
        return None
    return "prefilter: " + ", ".join(blocked)
...
def main() -> None:
    ...
    log.info("Experiment D: prefilter then Jev (volume, room to high, 15m up minutes)")
    ...
    ask_n = sum(1 for t in pending if prefilter_block(t) is None)
    log.info("Slots=%s  Jev calls after prefilter=%s", len(pending), ask_n)
