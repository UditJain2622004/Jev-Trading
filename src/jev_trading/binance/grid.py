"""Classic geometric long grid. Paper / backtest only — no live orders.
...
Fees: ``ROUND_TRIP_FEE`` (0.20%) is the full round-trip; half is charged
on each fill's notional. Accounting:
...
DEFAULT_COMBOS: tuple[GridLevers, ...] = (
    GridLevers("grid_0.5pct_8lvl", step_pct=0.5, levels=8),
    ...
    GridLevers("grid_1.0pct_10lvl_stop8", step_pct=1.0, levels=10, stop_pct=8.0),
    GridLevers("grid_0.8pct_12lvl", step_pct=0.8, levels=12),
)
...
def simulate(
    ...
    *,
    ...
    """Run the geometric long grid over ``klines``.
