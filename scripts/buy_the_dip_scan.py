...
from jev_trading.binance.buy_the_dip import (
    DEFAULT_COMBOS,
    DEFAULT_SYMBOLS,
    print_summary,
    scan_symbol,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Buy-the-dip lever scan on 1-month 1m data")
    parser.add_argument("--days", type=int, default=30)
    parser.add_argument("--notional", type=float, default=1000.0)
    parser.add_argument("--symbols", nargs="*", default=list(DEFAULT_SYMBOLS))
    parser.add_argument("--out", default="data/buy_the_dip/results.csv")
    parser.add_argument("--log-file", default="data/buy_the_dip/scan.log")
...
def setup_logging(log_file: Path) -> None:
    ...
    run_log = log_file.with_name(f"buy_the_dip_{stamp}.log")
    ...
    for name in ("buy_the_dip", "tp_sl_scan"):
...
def main() -> None:
    ...
    log.info("Buy-the-dip scan. No Jev. $%.0f clip, 0.20%% round-trip fee.", args.notional)
